...
- Fatma Himrane, born on March 27, 1947, in Jijel. She is proud of her Jijel roots but lived her life in Algiers (Belcourt, Bab El Oued, Duc Décart/Telemly, and currently Les Eucalyptus).
    ...
    recordingIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (recordingRef.current) {
          await recordingRef.current.stopAndUnloadAsync().catch((err) => { console.log(err); });
          const uri = recordingRef.current.getURI();
          if (uri && wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            const base64Audio = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
            const audioMsg = {
              realtimeInput: {
                mediaChunks: [{ mimeType: `audio/mp4`, data: base64Audio }]
              }
            };
        ...
        console.log("Audio chunk ignored");
      }
    }, 1500); 
  };
      ...
      {currentScreen === 'SETTINGS' && (
        <KeyboardAvoidingView style={styles.settingsContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <ScrollView contentContainerStyle={styles.settingsScroll}>
            <Text style={styles.settingsTitle}>Device Configuration</Text>
            ...
            <View style={styles.inputGroup}>
              <Text style={styles.label}>AI Voice Profile</Text>
              <View style={styles.radioGroup}>
...
const styles = StyleSheet.create({
  ...
  botAvatar: { width: 140, height: 140, borderRadius: 70, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 20, elevation: 10 },
